...
// Ce fichier est ignoré pour la couverture de test par Istanbul

import PropTypes from "prop-types";

// Composant Icon qui affiche un SVG en fonction de la propriété 'name'
const Icon = ({ name }) => {
  let icon; // Variable pour stocker le contenu du SVG en fonction de 'name'
  
  // Sélectionne le SVG à afficher en fonction de la valeur de 'name'
  switch (name) {
    case "twitch":
      // SVG pour l'icône Twitch
      icon = (
        <path
          data-testid="icon"
    ...
    case "youtube":
      ...
      break;
    case "close":
      // SVG pour l'icône de fermeture (close)
      icon = (
        <>
      ...
      break;
    default:
      icon = null; // Par défaut, pas d'icône (null)
  }

  // Rendu du composant SVG avec viewBox et contenu icon
  return (
    <svg
      width="36"
      height="36"
    ...
    >
      {icon} {/* Affiche l'icône sélectionnée */}
    </svg>
  );
};
...
Icon.propTypes = {
  name: PropTypes.string.isRequired, // 'name' doit être une chaîne (string) requise
};

export default Icon; // Exportation du composant Icon par défaut
